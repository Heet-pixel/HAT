...
const Departments = {
  ...
  async _fetchAndRender() {
    ...
    grid.innerHTML = Array(4)
      .fill('<div class="dept-box sk" style="height:160px"></div>')
      .join("");
    try {
  ...
  _renderBoxes(list) {
    ...
    if (!list.length) {
      grid.innerHTML =
        '<div class="empty" style="grid-column:1/-1">No departments yet. Create one above.</div>';
      return;
    }
    const icons = {
      "fa-laptop-code": "💻",
  ...
  async openDetail(deptId) {
    ...
    document.getElementById("dept-detail-icon").textContent =
      icons[dept.icon] || "🏛️";
  ...
  async _loadCourses(deptId, deptName) {
    ...
    grid.innerHTML = Array(3)
      .fill('<div class="course-card sk" style="height:120px"></div>')
      .join("");
    try {
      const d = await apiJson("/api/admin/courses?department=" + deptId);
      const list = d.data || d.courses || [];
      document.getElementById("dept-course-count").textContent =
        `${list.length} Course${list.length !== 1 ? "s" : ""}`;
      if (!list.length) {
        grid.innerHTML =
          '<div class="empty" style="grid-column:1/-1">No courses yet. Create one for this department.</div>';
        return;
      }
      const typeColors = {
        UG: "#e3f0ff",
  ...
  async save() {
    ...
    if (!name || !code) {
      UI.toast("Name and code required", "error");
      return;
    }
    try {
      const body = {
        name,
        code,
        establishedYear: year || undefined,
        icon,
        description: desc,
      };
      if (id) {
        await apiJson("/api/admin/departments/" + id, {
          method: "PUT",
          body: JSON.stringify(body),
        });
        UI.toast("Department updated");
      } else {
        await apiJson("/api/admin/departments", {
          method: "POST",
          body: JSON.stringify(body),
        });
  ...
  async deleteDept(id, name) {
    if (
      !confirm(
        `Delete "${name}"? This will also delete associated courses and subjects.`,
      )
    )
      return;
    try {
  ...
  openAddCourse() {
    ...
    document.getElementById("course-modal-title").textContent =
      `Add Course — ${dept.name}`;
    UI.openModal("modal-course");
  },

  _editCourse(course) {
    document.getElementById("course-id").value = course._id || course.id || "";
    document.getElementById("course-dept-id").value =
      course.departmentId || course.department?._id || "";
  ...
  async saveCourse() {
    ...
    if (!name) {
      UI.toast("Course name required", "error");
      return;
    }
    try {
      const body = {
      ...
      if (id) {
        await apiJson("/api/admin/courses/" + id, {
          method: "PUT",
          body: JSON.stringify(body),
        });
      ...
      if (Departments._selectedDept) {
        await Departments._loadCourses(
          Departments._selectedDept._id || Departments._selectedDept.id,
          Departments._selectedDept.name,
        );
      }
    } catch (e) {
      UI.toast(e.message || "Failed", "error");
